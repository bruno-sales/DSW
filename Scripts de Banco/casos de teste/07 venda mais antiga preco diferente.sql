...
CALL RegistraOrdemVenda(@idVendedor2, @idPersonagem, 1000, 200.0, @erro);
...
SELECT COUNT(id) INTO @contador FROM lancamentosPersonagem WHERE idUsuario = @idVendedor2 AND quantidade = 1000 AND precoUnitario = 200 AND operacao = 2;
CALL Debug("ERRO: lancamento de bloqueio do ativo no vendedor nao encontrado", @contador = 1);

-- registra uma terceira oferta de venda
CALL RegistraOrdemVenda(@idVendedor1, @idPersonagem, 1000, 250.0, @erro);
...
-- verifica se foi realizado o bloqueio do ativo no primeiro vendedor
SELECT COUNT(id) INTO @contador FROM lancamentosPersonagem WHERE idUsuario = @idVendedor1 AND quantidade = 1000 AND precoUnitario = 250 AND operacao = 2;
CALL Debug("ERRO: lancamento de bloqueio do ativo no vendedor nao encontrado", @contador = 1);
...
SELECT COUNT(id) INTO @contador FROM ofertas WHERE tipo = 0 AND status = 1 AND precoUnitario = 200.0 AND idUsuario = @IdVendedor;
CALL Debug("ERRO: a oferta de venda com preco mais baixo deveria ter sido executada", @contador = 1);

SELECT COUNT(id) INTO @contador FROM ofertas WHERE tipo = 0 AND status = 0;
CALL Debug("ERRO: duas ofertas de venda deveriam permanecer em aberto", @contador = 2);

SELECT COUNT(id) INTO @contador FROM ofertas WHERE tipo = 1 AND status = 1 AND precoUnitario = 350.0;
